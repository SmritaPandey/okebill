...
function generateInvoicePdf(data) {
    return new Promise((resolve, reject) => {
        ...
        if (data.clientEmail) {
            doc.text(data.clientEmail, leftMargin, y);
            y += 12;
        }
        if (data.clientAddress) {
            doc.text(data.clientAddress, leftMargin, y, { width: 250 });
            y += 12;
        }
        if (data.clientGstin) {
        ...
        ry += 14;
        doc.fontSize(9).font('Helvetica').fillColor('#4b5563');
        doc.text(`Issue Date: ${data.issueDate}`, 350, ry, { width: rightEdge - 350, align: 'right' });
        ry += 13;
        doc.text(`Due Date: ${data.dueDate}`, 350, ry, { width: rightEdge - 350, align: 'right' });
        ry += 13;
        if (data.placeOfSupplyName) {
            doc.text(`Place of Supply: ${data.placeOfSupplyName} (${data.placeOfSupply || ''})`, 350, ry, { width: rightEdge - 350, align: 'right' });
            ry += 13;
        }
        if (data.supplyType) {
            doc.text(`Supply Type: ${isInter ? 'Inter-State' : 'Intra-State'}`, 350, ry, { width: rightEdge - 350, align: 'right' });
            ry += 13;
        }
        if (data.reverseCharge) {
            doc.font('Helvetica-Bold').fillColor('#1E3A5F')
                .text('Reverse Charge: YES', 350, ry, { width: rightEdge - 350, align: 'right' });
            ry += 13;
        }
        // ════════════════════════════════════════════════
        // ITEMS TABLE
        ...
        if (data.irn) {
            ...
            doc.text(`IRN: ${data.irn}`, leftMargin, currentY);
            currentY += 10;
            if (data.ackNo) {
                doc.text(`Ack No: ${data.ackNo}  |  Ack Date: ${data.ackDate || ''}`, leftMargin, currentY);
                currentY += 10;
            }
            currentY += 4;
        }
        ...
        if (data.bankDetails && (data.bankDetails.accountNo || data.bankDetails.upiId)) {
            ...
            if (data.bankDetails.bankName) {
                doc.text(`Bank: ${data.bankDetails.bankName}${data.bankDetails.branch ? ' — ' + data.bankDetails.branch : ''}`, leftMargin, currentY);
                currentY += 11;
            }
            if (data.bankDetails.accountNo) {
                doc.text(`A/C No: ${data.bankDetails.accountNo}`, leftMargin, currentY);
                currentY += 11;
            }
            if (data.bankDetails.ifsc) {
                doc.text(`IFSC: ${data.bankDetails.ifsc}`, leftMargin, currentY);
                currentY += 11;
            }
            if (data.bankDetails.upiId) {
                doc.text(`UPI: ${data.bankDetails.upiId}`, leftMargin, currentY);
                currentY += 11;
            }
            currentY += 6;
        }
        ...
        doc.text('Powered by OkeBill', leftMargin + pageWidth / 2, footerY, { width: pageWidth / 2, align: 'right' });
        doc.end();
    });
}
function generateProposalPdf(data) {
    return new Promise((resolve, reject) => {
        ...
        doc.text('Generated by OkeBill — Simple | Hisab | Accurate', 50, footerY, { width: pageWidth, align: 'center' });
        doc.end();
    });
}
