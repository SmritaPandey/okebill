...
const gsp_1 = require("../services/gsp");
...
router.get('/verify/:gstin', async (req, res) => {
    try {
        ...
        if (masterIndiaApiKey) {
            ...
            try {
                const altResponse = await fetch(`https://commonapi.mastersindia.co/commonapis/searchgstin?gstin=${gstin}`, {
                    headers: {
                        'Accept': 'application/json',
                        'User-Agent': 'OkeBill/1.0',
                    },
...
router.post('/e-invoice/generate', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (!user?.gstin) {
            res.status(400).json({ message: 'Company GSTIN not configured. Set it in Settings.' });
            return;
        }
        if (!invoice.client.gstin) {
            res.status(400).json({ message: 'Client GSTIN is required for E-Invoice.' });
            return;
        }
        ...
        const result = await gsp_1.gspService.generateEInvoice({
...
router.post('/e-invoice/cancel', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (!invoice.irn) {
            res.status(400).json({ message: 'No IRN found on this invoice' });
            return;
        }
        const result = await gsp_1.gspService.cancelEInvoice(invoice.irn, reason || '1', remark || 'Cancelled');
        if (result.success) {
            await prisma_1.default.invoice.update({
                where: { id: invoice.id },
...
router.post('/eway-bill/generate', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const user = await prisma_1.default.user.findUnique({ where: { id: req.userId } });
        if (!user?.gstin) {
            res.status(400).json({ message: 'Company GSTIN not configured' });
            return;
        }
        const items = Array.isArray(invoice.items) ? invoice.items : [];
        const isInter = invoice.supplyType === 'inter';
        ...
        const invoiceDate = new Date(invoice.issueDate);
        const formattedDate = `${String(invoiceDate.getDate()).padStart(2, '0')}/${String(invoiceDate.getMonth() + 1).padStart(2, '0')}/${invoiceDate.getFullYear()}`;
        const TRANSPORT_MODE_MAP = { road: 1, rail: 2, air: 3, ship: 4 };
        const result = await gsp_1.gspService.generateEWayBill({
            supplyType: 'O',
            subSupplyType: 1,
            docType: 'INV',
...
router.put('/eway-bill/:ewbNo/vehicle', auth_1.authMiddleware, async (req, res) => {
    try {
        const { ewbNo } = req.params;
        const { vehicleNo, transportMode, reason } = req.body;
        const TRANSPORT_MODE_MAP = { road: 1, rail: 2, air: 3, ship: 4 };
        const result = await gsp_1.gspService.updateEWayBillVehicle(String(ewbNo), vehicleNo, TRANSPORT_MODE_MAP[transportMode] || 1, reason || 4);
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
router.post('/eway-bill/:ewbNo/cancel', auth_1.authMiddleware, async (req, res) => {
    try {
        const { ewbNo } = req.params;
        const { reason, remark } = req.body;
        const result = await gsp_1.gspService.cancelEWayBill(String(ewbNo), reason || 4, remark || 'Cancelled');
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
router.get('/gstr1/export', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await gsp_1.gspService.exportGstr1Data(invoices, period);
...
router.get('/status', auth_1.authMiddleware, async (_req, res) => {
    ...
    res.json({
        gspConfigured: gsp_1.gspService.isConfigured(),
        provider: providerNames[gsp_1.gspService.getProvider()] || 'Unknown',
        providerKey: gsp_1.gspService.getProvider(),
        features: {
            eInvoice: true,
            eWayBill: true,
            gstr1Export: true,
