...
router.post('/', async (req, res) => {
    try {
        ...
        const staff = await prisma_1.default.staffMember.create({
            data: {
                userId: req.userId,
                name,
                phone: phone || null,
...
router.put('/:id', async (req, res) => {
    try {
        const existing = await prisma_1.default.staffMember.findFirst({
        ...
        const staff = await prisma_1.default.staffMember.update({
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await prisma_1.default.staffMember.delete({ where: { id: Number(req.params.id) } });
        res.status(204).send();
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.get('/attendance/daily', async (req, res) => {
    try {
        ...
        const allStaff = await prisma_1.default.staffMember.findMany({
