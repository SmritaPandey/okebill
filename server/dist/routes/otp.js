...
function generateOtp() {
    return String(Math.floor(100000 + crypto_1.default.randomInt(900000)));
}
// ─── POST /otp/send ──────────────────────────────────────
router.post('/send', async (req, res) => {
    try {
        ...
        if (type === 'phone') {
            ...
            const recentCount = await prisma_1.default.otpVerification.count({
                where: {
                    target: cleaned,
                    targetType: 'phone',
            ...
            if (smsProvider === '2factor' || process.env.TWOFACTOR_API_KEY) {
                ...
                try {
                    const tfResponse = await fetch(`https://2factor.in/API/V1/${apiKey}/SMS/+91${cleaned}/${otpCode}/OkeBill+OTP+is+${otpCode}+valid+for+${OTP_EXPIRY_MINUTES}+min`, { signal: AbortSignal.timeout(10000) });
