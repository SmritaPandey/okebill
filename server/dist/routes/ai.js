...
async function getBillingContext(userId) {
    ...
    const [clients, allInvoices, paymentsThisMonth, paymentsLastMonth, allPayments, proposals, contracts, recentPayments, emailLogs,] = await Promise.all([
        prisma_1.default.client.findMany({ where: { userId }, select: { id: true, name: true, contactEmail: true, phone: true, address: true, createdAt: true } }),
        ...
        prisma_1.default.payment.aggregate({ where: { userId, paymentDate: { gte: startOfMonth } }, _sum: { amount: true }, _count: true }),
        prisma_1.default.payment.aggregate({ where: { userId, paymentDate: { gte: startOfLastMonth, lte: endOfLastMonth } }, _sum: { amount: true }, _count: true }),
        prisma_1.default.payment.aggregate({ where: { userId }, _sum: { amount: true }, _count: true }),
        prisma_1.default.proposal.findMany({
            where: { userId },
...
${context.clients.map((c) => `• ${c.name} — ${c.contactEmail}${c.phone ? `, ${c.phone}` : ''}${c.address ? ` (${c.address})` : ''}`).join('\n')}

🏆 TOP CLIENTS BY REVENUE:
${context.topClients.length > 0 ? context.topClients.map((c) => `• ${c.name}: $${c.revenue}`).join('\n') : '• No payments recorded yet'}

⚠️ TOP CLIENTS BY OUTSTANDING BALANCE:
${context.topOwing.length > 0 ? context.topOwing.map((c) => `• ${c.name}: $${c.outstanding} outstanding${c.overdueCount > 0 ? ` (${c.overdueCount} overdue)` : ''}`).join('\n') : '• All clear!'}

🔴 OVERDUE INVOICES:
${context.overdueInvoices.length > 0
        ? context.overdueInvoices.map((i) => `• ${i.invoiceNumber}: $${i.total} from ${i.clientName} — ${i.daysOverdue} days overdue (due ${i.dueDate})`).join('\n')
        : '• None! All invoices are on track.'}

📄 RECENT INVOICES:
${context.recentInvoices.map((i) => `• ${i.invoiceNumber}: $${i.total} → ${i.clientName} [${i.status}]`).join('\n')}
...
${context.proposals.map((p) => `• "${p.title}" → ${p.clientName}: $${p.total} [${p.status}]`).join('\n')}
...
${context.contracts.map((c) => `• "${c.title}" → ${c.clientName}: $${c.value} [${c.status}]`).join('\n')}

${context.expiringContracts.length > 0 ? `🔶 CONTRACTS EXPIRING SOON:\n${context.expiringContracts.map((c) => `• "${c.title}" → ${c.clientName}: expires in ${c.daysUntilExpiry} days`).join('\n')}` : ''}

💰 RECENT PAYMENTS:
${context.recentPayments.map((p) => `• $${p.amount} via ${p.paymentMethod} on ${p.paymentDate} (Ref: ${p.reference || 'N/A'})`).join('\n') || '• No recent payments'}

📧 RECENT EMAIL ACTIVITY:
${context.recentEmails.map((e) => `• ${e.subject} → ${e.to} [${e.status}] on ${e.date}`).join('\n') || '• No emails sent yet'}
...
router.post('/chat', async (req, res) => {
    try {
        ...
        // Save user message
        await prisma_1.default.aiChatMessage.create({
        ...
        if (apiKey) {
            try {
                ...
                const chatHistory = [
                    ...
                    { role: 'model', parts: [{ text: `Understood! I'm your OkeBill billing assistant for ${user.firstName}. I have full real-time access to your billing data. How can I help you today?` }] },
...
function generateFallbackResponse(message, context) {
    ...
    if (/^(hi|hello|hey|good morning|good afternoon|good evening|howdy|sup|what's up)\b/.test(lower)) {
        ...
        if (context.summary.overdueCount > 0) {
            greeting += `⚠️ **Heads up:** You have ${context.summary.overdueCount} overdue invoice(s) totaling $${context.overdueInvoices.reduce((s, i) => s + parseFloat(i.total), 0).toFixed(2)}. Want me to help you follow up?\n\n`;
        }
    ...
    if (/overdue|late|past\s*due|unpaid|outstanding|owes?|owed/.test(lower)) {
        ...
        context.overdueInvoices.forEach((i) => {
        ...
        const totalOverdue = context.overdueInvoices.reduce((s, i) => s + parseFloat(i.total), 0);
    ...
    if (/revenue|summary|report|how much|dashboard|analytics|earnings|income|money|profit|stats|statistics|financial|performance|overview/.test(lower)) {
        ...
        resp += `• Overdue amount: $${context.overdueInvoices.reduce((s, i) => s + parseFloat(i.total), 0).toFixed(2)}\n\n`;
        if (context.topClients.length > 0) {
            resp += `**🏆 Top Clients by Revenue:**\n`;
            context.topClients.forEach((c, i) => {
    ...
    if (/(send|email|mail)\s*(an?\s*)?(invoice|bill|reminder)/.test(lower) || /send\s*reminder/.test(lower)) {
        ...
        const unsent = context.recentInvoices.filter((i) => i.status === 'draft');
        if (unsent.length === 0) {
    ...
    if (/(record|log|add|register|receive)\s*(a?\s*)?payment/.test(lower) || /got\s*paid|received\s*payment/.test(lower)) {
        const payable = context.recentInvoices.filter((i) => ['sent', 'overdue', 'draft'].includes(i.status));
        if (payable.length === 0) {
    ...
    if (/download|pdf|export|print/.test(lower)) {
        if (/proposal/.test(lower)) {
            if (context.proposals.length === 0)
                return `No proposals found to download.`;
            let resp = `📄 **Download Proposal PDF**\n\n`;
            context.proposals.slice(0, 5).forEach((p) => {
        ...
        if (context.recentInvoices.length === 0)
            return `No invoices found to download.`;
        let resp = `📄 **Download Invoice PDF**\n\nRecent invoices:\n\n`;
        context.recentInvoices.slice(0, 8).forEach((i) => {
    ...
    if (/^(list|show|view|see|all|my)\s*(all\s*)?(recent\s*)?(invoices?|bills?)/.test(lower) || lower === 'invoices') {
        ...
        context.recentInvoices.slice(0, 10).forEach((i) => {
    ...
    if (/clients?|customers?|contacts?/.test(lower) && !/create|add|new/.test(lower)) {
        ...
        context.clients.forEach((c) => {
            const outstanding = context.topOwing.find((o) => o.name === c.name);
            const topRev = context.topClients.find((t) => t.name === c.name);
            resp += `• **${c.name}** — ${c.contactEmail}`;
            if (topRev)
                resp += ` · Revenue: $${topRev.revenue}`;
            if (outstanding)
                resp += ` · Outstanding: $${outstanding.outstanding}`;
            resp += '\n';
        });
        resp += `\n\`\`\`action\n{"type": "navigate", "data": {"page": "/clients"}}\n\`\`\``;
        return resp;
    ...
    if (/contracts?/.test(lower)) {
        if (/expir|renew/.test(lower)) {
            ...
            context.expiringContracts.forEach((c) => {
    ...
    if (/payments?|paid|received|transactions?|payment\s*history/.test(lower) && !/record|log|add/.test(lower)) {
        ...
        if (context.recentPayments.length > 0) {
            ...
            context.recentPayments.forEach((p) => {
    ...
    const clientMatch = context.clients.find((c) => lower.includes(c.name.toLowerCase()));
    if (clientMatch) {
        const clientInvoices = context.recentInvoices.filter((i) => i.clientId === clientMatch.id);
        const clientProposals = context.proposals.filter((p) => p.clientId === clientMatch.id);
        const clientContracts = context.contracts.filter((c) => c.clientId === clientMatch.id);
        const clientOwing = context.topOwing.find((o) => o.name === clientMatch.name);
        const clientRevenue = context.topClients.find((t) => t.name === clientMatch.name);
        let resp = `👤 **Client Profile: ${clientMatch.name}**\n\n`;
        resp += `📧 ${clientMatch.contactEmail}`;
        if (clientMatch.phone)
            resp += ` · 📞 ${clientMatch.phone}`;
        if (clientMatch.address)
            resp += `\n📍 ${clientMatch.address}`;
        resp += '\n\n';
        ...
        if (clientProposals.length > 0) {
            ...
            resp += clientProposals.map((p) => `"${p.title}" [${p.status}]`).join(', ');
            resp += '\n\n';
        }
        if (clientContracts.length > 0) {
            resp += `**Contracts (${clientContracts.length}):** `;
            resp += clientContracts.map((c) => `"${c.title}" [${c.status}]`).join(', ');
            resp += '\n';
        }
        return resp;
    }
    ...
    if (/thank|thanks|great|awesome|perfect|goodbye|bye|that'?s? all/.test(lower)) {
        return `😊 You're welcome! I'm here anytime you need help with billing. Have a great day!\n\n${context.summary.overdueCount > 0 ? `💡 **Reminder:** Don't forget about the ${context.summary.overdueCount} overdue invoice(s) worth $${context.overdueInvoices.reduce((s, i) => s + parseFloat(i.total), 0).toFixed(2)}.` : '✅ Your billing is in great shape!'}`;
    }
...
router.get('/history', async (req, res) => {
    try {
        const messages = await prisma_1.default.aiChatMessage.findMany({
...
router.delete('/history', async (req, res) => {
    try {
        await prisma_1.default.aiChatMessage.deleteMany({ where: { userId: req.userId } });
        res.status(204).send();
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.get('/suggestions', async (req, res) => {
    try {
        ...
        suggestions.push({ label: 'Revenue Report', icon: '📊', message: 'Show my revenue summary' }, { label: 'Create Invoice', icon: '➕', message: 'Create a new invoice' }, { label: 'Send Reminders', icon: '📧', message: 'Send payment reminders' }, { label: 'Aging Report', icon: '🏦', message: 'Show aging report' }, { label: 'Cash Flow', icon: '💰', message: 'Show cash flow analysis' });
