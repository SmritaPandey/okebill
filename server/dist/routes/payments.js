...
router.post('/record', async (req, res) => {
    try {
        ...
        const paymentCount = await prisma_1.default.payment.count({ where: { userId: req.userId } });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await prisma_1.default.payment.delete({ where: { id: Number(req.params.id) } });
        // Recalculate invoice status
        const remainingPayments = await prisma_1.default.payment.aggregate({
            where: { invoiceId: payment.invoiceId },
            _sum: { amount: true },
        });
        const invoice = await prisma_1.default.invoice.findUnique({ where: { id: payment.invoiceId } });
        if (invoice) {
...
router.post('/gateway/create-order', async (req, res) => {
    try {
        ...
        if (balance <= 0) {
            res.status(400).json({ message: 'Invoice already fully paid' });
            return;
        }
        const order = await razorpay_1.razorpayService.createOrder(balance, invoice.invoiceNumber, {
        ...
        res.json({
            ...
            keyId: razorpay_1.razorpayService.getKeyId() || 'rzp_test_demo',
...
router.post('/gateway/verify', async (req, res) => {
    try {
        ...
        const isValid = await razorpay_1.razorpayService.verifyPayment({
            razorpay_order_id,
            razorpay_payment_id,
            razorpay_signature,
        });
        ...
        const paymentDetails = await razorpay_1.razorpayService.getPayment(razorpay_payment_id);
        ...
        const allPayments = await prisma_1.default.payment.aggregate({
            where: { invoiceId: invoice.id },
            _sum: { amount: true },
        });
...
router.post('/gateway/payment-link', async (req, res) => {
    try {
        ...
        const link = await razorpay_1.razorpayService.createPaymentLink({
...
router.post('/gateway/refund', async (req, res) => {
    try {
        ...
        const refund = await razorpay_1.razorpayService.createRefund(razorpayPaymentId, refundAmount, reason);
...
router.get('/gateway/status', async (_req, res) => {
    ...
    res.json({
        razorpay: {
            configured: razorpay_1.razorpayService.isConfigured(),
            provider: 'Razorpay',
            keyId: razorpay_1.razorpayService.getKeyId() ? '***' + razorpay_1.razorpayService.getKeyId().slice(-4) : null,
        },
        payg: {
        ...
        activeGateway: paygConfigured ? 'payg' : razorpay_1.razorpayService.isConfigured() ? 'razorpay' : 'none',
...
router.post('/payg/create-order', async (req, res) => {
    try {
        const { invoiceId } = req.body;
        const invoice = await prisma_1.default.invoice.findFirst({
            where: { id: Number(invoiceId), userId: req.userId },
        ...
        const user = await prisma_1.default.user.findUnique({ where: { id: req.userId } });
        const order = await (0, payg_1.createPaygOrder)({
...
router.post('/payg/verify', async (req, res) => {
    try {
        const { orderKeyId, invoiceId } = req.body;
        if (!orderKeyId) {
            res.status(400).json({ message: 'orderKeyId is required' });
            return;
        }
        const status = await (0, payg_1.getPaygOrderStatus)(orderKeyId);
...
router.get('/payg/status/:orderKeyId', async (req, res) => {
    try {
        const status = await (0, payg_1.getPaygOrderStatus)(String(req.params.orderKeyId));
        res.json({
